...
<body>
  <header class="text-center border-bottom p-1 d-flex justify-content-between align-items-center px-3">
    ...
    <?php
      if(!$userExists){
        $href = "/v/users/register.php";
        $text = "Sign-up";
      } else {
        $href = "/v/users/login.php";
        $text = "Login";
      }
    ?>
    ...
    <div>
      <?php
        if(!isset($_SESSION['id'])){
          ...
          ?>
            <a href="/v/users/edit.php" style="margin-left: 10px;">Edit Profile</a>
            <a href="/v/notices/register.php" style="margin-left: 10px;">Register</a>
            <a href="/api/users/exit.php" style="margin-left: 10px;">Exit</a>
          <?php
        }
      ?>
    </div>
          ...
          ?>
            <div class="card w-100 mt-2 shadow">
              <div class="card-body">
                ...
                <h6 class="card-subtitle mb-2 text-body-secondary"><?= date('d/m/Y H:i', strtotime($notice['creation_date'])) ?></h6>
                <p class="card-text"><?= htmlspecialchars($notice['description']) ?></p>
                  ...
                  <?php
                    ...
                    if(isset($_SESSION['id'])){
                      ?>
                        <a href="/v/notices/edit.php?notice=<?= $notice['id'] ?>">Edit</a>
                        <a href="/api/notices/delete.php?notice=<?= $notice['id'] ?>">Delete</a>
                      <?php
                    }
                  ?>
              </div>
